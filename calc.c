...
void ler_arquivo(){
    ...
    FILE *input;
    expressao[0] = '(';
    printf("Enter name of input file:"); // Perguntando como nomear arquivos em cada volta
    scanf(" %s", in);
    input = fopen(in, "r");    
    int i = 1;
    while (feof(input) == 0) {
        fscanf(input, "%c", &expressao[i]);
        i++;
    }
    printf("%d\n",i);
    fclose(input);
    int N = ((strlen(expressao))-1);
    expressao[N] = ')';
...
void menu(){
    int opcao;
    printf("Escolha: 1 - Calculo simples 2 - Calculo expressão 0 - Sair\n");
    scanf("%d", &opcao);
    while (opcao != 0){
        if (opcao == 1){
            ler();
